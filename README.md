...
3. Programming Practice. Take these two individual sketches, and add color in some interactive way:
  - Mouse-tracking Circles
    ...
    void setup() {
      ...
      mouseX = floor(random(width));
      mouseY = floor(random(height));
    }
    ...
    void draw() {
      ...
      for (int i = 0; i < GRID_SIZE; ++i) {
        for (int j = 0; j < GRID_SIZE; ++j) {
          float ellipseSize = min(40, 1000.0/sqrt(pow(mouseX-gridToCoordinates(i), 2) + pow(mouseY-gridToCoordinates(j), 2)));
